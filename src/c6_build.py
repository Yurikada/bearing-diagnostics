...
<div class="cards">
  ...
  2本×6.8日で観測された誤報0件(2層合成・条件全体に3点確定・全4ch比較)。合成の定義に
  よっては後期4件が残る(本文に開示)。独立 run での転移テストは⑤参照</div></div>
...
不変)。比較は全4チャンネルで行い、B3 を誤報統計から除外した理由(健全期から別の異常
シグネチャを持つ個体 — 本文C1〜C2で確認)も開示した。ただしセンサ感度差・伝達経路・
同時故障ではこの前提が崩れるため、チャンネル間校正が前提になる。
なお**2層の合成は自明ではない**: 持続条件(3点確定)を合成条件のどこに掛けるかで結果が
変わり、単点ゲートの合成では後期4クラスタが残存、条件全体に3点確定を掛けた合成で
0件となった(両定義と、後者が前者の観測後に定義された順序を開示)。<b>多センサ系の異常検知は「何が起きたか」と「どこで
...
<tr><td>最終盤の B3</td><td>尖度が数十→3〜8 へ低下しながら RMS が 0.58g へ急増 — 剥離全面化によるガウス化を完全な形で観測</td><td class="won">C1 理論と整合</td></tr>
<tr><td>B4(低確度と予想)</td><td>2×BSF −0.3〜−0.6% に一貫した位置指紋、周波数分離型帰属で自ch勝率 73%</td><td class="won">予想外の成立</td></tr>
<tr><td>B3(高確度と予想)</td><td>エンベロープ線/床は中央値 1.42(Set2 の 1/10 以下)、側帯も床レベル</td><td class="lost">不成立</td></tr>
<tr><td>B3 窓の帰属</td><td>自ch勝率 35%(2故障同時で部分破綻 — 事前に本人が設計した破綻シナリオの現実化)</td><td class="lost">部分破綻</td></tr>
<tr><td>健全chの早期誤報</td><td>0 件(Set2 から再現)</td><td class="won">転移</td></tr>
</table>
<p>B3 窓では 301Hz の線が観測されたが、窓の分離性検証が回転高調波(9×fr = 300.0Hz が
窓内)を見ていなかったことが判明。fr を実測して判別する測定を設計・実行したが、
fr 線族の S/N 不足で要求精度に届かず、<b>301Hz の起源同定は判定不能</b>と確定した。
教訓: 窓の分離性検証には k·fr 高調波族を必須で含める / 判別測定にも成立条件がある。</p>
<p><b>結論</b>: 開発 run の検出器は「そのまま」は独立 run に転移しない。転移しなかった
箇所(2故障同時の帰属・分位点の脆弱性・帯域スキャンの前提)はいずれも開発中に弱点として
特定していたものの現実化であり、弱点予測の的中を転移テストの成果として記録する。</p>

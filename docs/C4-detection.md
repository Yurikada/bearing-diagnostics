...
- **検出器の因果性**(本ステージの新概念): 帯域 3–6.5kHz は day5 の櫛を見て特定した
  ...
  - **因果帯域**: 各時点までの情報のみ — 帯域内尖度最大の 1kHz 窓(固定幅1kHz窓の
    因果的な帯域尖度スキャン。古典的 Kurtogram とは異なる。本人の C3 設計)を
    直近5点の中央値で平滑化(breakdown point 50% で
    単発ノイズを遮断 — 本人説明)。
...
注: 表の時刻は「系列が閾値を超え始めた最初のスナップショット」。警報が確定するのは
3点目(+約20分)で、C5 のリードタイム報告は確定時刻基準に統一した(レビューで修正)。
...
**236Hz へ固定**(4.5≤day<6.5 の有効 288 点中 288 点 — 区間定義を明示して再集計)。床のピーク位置は
